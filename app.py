...
def provider_dashboard():
    ...
    filter_status = request.args.get('filter', 'pending')  # Default to pending
    ...
    try:
        base_query = Appointment.query.join(User, Appointment.user_id == User.id).filter(
            User.role == 'patient',
            (User.institution == institution) | ((User.institution.is_(None)) & (institution == 'Sample University')),
            ((Appointment.provider_id.is_(None)) | (Appointment.provider_id == session['user_id']))
        )
        
        if filter_status == 'pending':
            filtered_appointments = base_query.filter(Appointment.status.in_(['booked', 'pending'])).order_by(Appointment.date.asc(), Appointment.time.asc()).all()
        elif filter_status == 'accepted':
            filtered_appointments = base_query.filter(Appointment.status == 'accepted').order_by(Appointment.date.desc(), Appointment.time.desc()).all()
        elif filter_status == 'rejected':
            filtered_appointments = base_query.filter(Appointment.status == 'rejected').order_by(Appointment.updated_at.desc()).all()
        else:  # 'all'
            filtered_appointments = base_query.order_by(Appointment.date.desc(), Appointment.time.desc()).all()
            
        pending_appointments = filtered_appointments
    except Exception as e:
...
def schedule():
    if request.method == 'POST':
        ...
        try:
            ...
            # Emit real-time notification to provider(s)
            try:
                if provider_id:
                    ...
                    # Notify all providers in the institution
                    institution = session.get('user_institution')
                    providers = User.query.filter_by(role='provider', institution=institution).all()
                    for prov in providers:
                        socketio.emit('new_appointment', {
                            ...
                            'time': time_str,
                            'type': appointment_type
                        }, room=f'user_{prov.id}')
            except Exception as socket_error:
                logger.warning(f"Failed to emit new appointment notification: {socket_error}")
    ...
    # For GET: provide a list of providers in the same institution for optional assignment
    providers = []
    try:
        providers = User.query.filter_by(role='provider', institution=session.get('user_institution')).all()
        # Fallback to all providers if institution not set
        if not providers:
            providers = User.query.filter_by(role='provider').all()
    except Exception as e:
        logger.error(f"Error fetching providers for schedule: {e}")
        providers = []
...
def accept_appointment(appointment_id):
    ...
    # Only allow accepting if unassigned or assigned to this provider and status is booked/pending
    if appt.status not in ['booked', 'pending']:
        flash('Appointment is not pending.', 'error')
        return redirect(url_for('provider_dashboard'))
    if appt.provider_id is not None and appt.provider_id != session['user_id']:
        abort(403)
    try:
        appt.provider_id = session['user_id']
        ...
        try:
            socketio.emit('appointment_accepted', {
                ...
                'time': appt.time,
                'provider_name': session.get('user_name', 'Provider')
            }, room=f'user_{appt.user_id}')
        except Exception as socket_error:
            logger.warning(f"Failed to emit acceptance notification: {socket_error}")
        
        flash('Appointment accepted.', 'success')
    except Exception as e:
        db.session.rollback()
        logger.error(f"Failed to accept appointment {appointment_id}: {e}")
        flash('Failed to accept appointment.', 'error')
    return redirect(url_for('provider_dashboard'))

@app.route('/appointments/<int:appointment_id>/reject', methods=['POST'])
@login_required
@role_required('provider')
def reject_appointment(appointment_id):
    appt = Appointment.query.get_or_404(appointment_id)
    # Only allow rejecting if unassigned or assigned to this provider and status is booked/pending
    if appt.status not in ['booked', 'pending']:
        flash('Appointment is not pending.', 'error')
        return redirect(url_for('provider_dashboard'))
    if appt.provider_id is not None and appt.provider_id != session['user_id']:
        abort(403)
    try:
        # Mark as rejected; keep provider_id if already assigned
        rejection_reason = request.form.get('rejection_reason', '').strip()
        appt.status = 'rejected'
        appt.rejection_reason = rejection_reason if rejection_reason else None
        db.session.commit()
        ...
        try:
            ...
            logger.warning(f"Failed to emit rejection notification: {socket_error}")
        
        flash('Appointment rejected.', 'success')
    except Exception as e:
        db.session.rollback()
        logger.error(f"Failed to reject appointment {appointment_id}: {e}")
...
# --- SocketIO User Connection Handlers for Appointment Notifications ---
@socketio.on('connect')
def handle_connect():
    """Handle user connection and join their personal room for notifications"""
